...
while True:
    ...
    if move[0] == "inventory":
        while True:
            for i in range(len(inventory)):
                if (inventory[i] == "note"):
                    print(f"{i+1}: {inventory[i]} (Press R to read)")
                else: 
                    print(f"{i+1}: {inventory[i]}")
            inp = input("E to exit inventory\n>")
            if (inp == "R" and "note" in inventory):
                print("\nBEWARE OF THE MONSTER LURKING IN THE KITCHEN\n")
            elif (inp.lower() == "q"):
                break;
    ...
    if move[0] == 'get':
        ...
        if "item" in rooms[currentRoom] and move[1] in rooms[currentRoom]['item']:
            ...
            print(move[1] + ' got!')
            # delete the item key:value pair from the rooms dictionary
